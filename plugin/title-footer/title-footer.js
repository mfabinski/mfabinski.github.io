...
title_footer.initialize=function(title,background)
{
  ...
  var today = dd+'.'+mm+'.'+yyyy;
	a_date.setAttribute("id","footer_date");
	a_date.appendChild(document.createTextNode(today));
	title_footer_p.appendChild(a_date);
	//title
	var a_title=document.createElement('a');
	a_title.setAttribute('href','#/0');
	a_title.appendChild(document.createTextNode(this.title));
	title_footer_p.appendChild(a_title);
	var img=document.createElement('img');
	img.setAttribute("src","img/logo.png");
	img.setAttribute("id","footer_image");
	title_footer_p.appendChild(img);
	//slidenumber
	var a_slidenumber=document.createElement('a');
  var slidenumber = (Reveal.getIndices().h+1) + " : " + (Reveal.getIndices().v+1);
	a_slidenumber.setAttribute("id","footer_slidenumber");
	a_slidenumber.appendChild(document.createTextNode(slidenumber));
	title_footer_p.appendChild(a_slidenumber);

	var div_class_reveal=document.querySelectorAll('.reveal')[0];
	div_class_reveal.appendChild(title_footer);

	Reveal.addEventListener( 'slidechanged', function( event ) {
		var a=document.getElementById('footer_slidenumber');
		a.firstChild.data = (Reveal.getIndices().h+1) + " : " + (Reveal.getIndices().v+1);
	} );
};
